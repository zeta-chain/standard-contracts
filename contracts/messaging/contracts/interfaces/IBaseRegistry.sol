...
interface IBaseRegistryEvents {
    ...
    event RegistryManagerChanged(
        address oldRegistryManager,
        address newRegistryManager
    );
}
...
interface IBaseRegistry is IBaseRegistryErrors, IBaseRegistryEvents {
    ...
    function getChainInfo(
        uint256 chainId
    ) external view returns (address gasZRC20, bytes memory registry);
    ...
    function getContractInfo(
        uint256 chainId,
        string calldata contractType
    ) external view returns (bool active, bytes memory addressBytes);

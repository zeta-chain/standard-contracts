...
const main = async (args, hre) => {
    ...
    const contract = await factory.deploy(args.gateway, signer.address, ...(args.uniswapRouter ? [args.uniswapRouter] : []), ...(args.router ? [args.router] : []), ...(args.contractRegistry ? [args.contractRegistry] : []));
...
(0, config_1.task)("deploy", "Deploy the contract", main)
    .addFlag("json", "Output the result in JSON format")
    .addOptionalParam("name", "The contract name to deploy", "Example")
    .addOptionalParam("gateway", "Gateway address (default: ZetaChain Gateway)", "0x6c533f7fe93fae114d0954697069df33c9b74fd7")

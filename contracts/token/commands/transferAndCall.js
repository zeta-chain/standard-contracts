...
exports.transferAndCall = void 0;
const commander_1 = require("commander");
const ethers_1 = require("ethers");
const common_1 = require("./common");
const normalizeBytes = (input) => {
    if (!input || input === "0x" || input === "")
        return "0x";
    if (input.startsWith("0x") || input.startsWith("0X")) {
        const hex = input.slice(2);
        return hex.length % 2 === 0 ? "0x" + hex : "0x0" + hex;
    }
    return ethers_1.ethers.utils.hexlify(ethers_1.ethers.utils.toUtf8Bytes(input));
};
const main = async (opts) => {
    ...
    if (!ethers_1.ethers.utils.isAddress(opts.to)) {
        throw new Error("Invalid destination ZRC-20 address.");
    }
    if (!ethers_1.ethers.utils.isAddress(opts.from)) {
        throw new Error("Invalid source contract address.");
    }
    const { abi } = (0, common_1.loadContractArtifacts)(opts.name);
    const contract = new ethers_1.ethers.Contract(opts.from, abi, signer);
    const amountInWei = ethers_1.ethers.utils.parseUnits(String(opts.amount), 18);
    const approveTx = await contract.approve(opts.from, amountInWei, {
        gasLimit: opts.gasLimit,
    });
    await approveTx.wait();
    const gasAmount = ethers_1.ethers.utils.parseUnits(String(opts.gasAmount), 18);
    const receiver = opts.receiver || signer.address;
    const fnSignature = opts.function;
    const iface = new ethers_1.ethers.utils.Interface([`function ${fnSignature}`]);
    ...
    const tx = await contract.transferCrossChainAndCall(opts.to, receiver, String(opts.amount), encodedMessage, {
        gasLimit: opts.gasLimit,
        value: gasAmount,
    });
...
exports.transferAndCall = new commander_1.Command("transfer-and-call")
    ...
    .option("-t, --to <address>", "Destination ZRC-20", "0x0000000000000000000000000000000000000000")
    .requiredOption("-F, --function <signature>", "Function signature, e.g. hello(bytes)")
